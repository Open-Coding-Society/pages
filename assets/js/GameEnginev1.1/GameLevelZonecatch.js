...
class ZoneCatchOverlay {
    ...
    _showAiBriefing() {
        ...
        const spriteData = {
            ...
            chatHistory: [],
            dialogues: [
                "🎯 Welcome to Zone Catch! Each round, two coloured zones appear — a banner tells you which colour is SAFE. Move into the safe zone with W, A, S, D before the timer runs out. Standing outside it when time ends = eliminated! Between rounds, mouse-click the POWER CORE at the top of the arena for bonus coins (every 4 clicks = +1 coin). Survive, and from Round 6 a golden GATE will appear somewhere on the map. Reach it and press E to escape early. Ask me anything, then click Start Level!"
            ],
            knowledgeBase: {
...
class GameLevelZonecatch {
    constructor(gameEnv) {
        ...
        // The ZoneCatch overlay uses pointer-events:none, so canvas clicks pass
        // through to this Clicker NPC. Every 4 clicks = +1 bonus coin, cap 20.
