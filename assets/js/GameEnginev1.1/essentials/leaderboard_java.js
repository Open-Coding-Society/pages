/**
 * Leaderboard (Java Variant) - Dual-Mode Leaderboard Widget for GameEngine v1.1
 *
 * Mirrors leaderboard_python.js functionality while targeting the Spring
 * /api/events backend.
 *
 * Uses Spring via javaURI only.
 */
...
export default class Leaderboard {
	constructor(gameControl = null, options = {}) {
		...
		// Spring backend only for this Java variant.
		this.backendURI = javaURI ? String(javaURI).trim() : '';
	...
	fetchLeaderboard() {
		...
		return this._fetchFromBackends('/api/events/SCORE_COUNTER', fetchOptions)
			.then((res) => res.json())
			.then((data) => {
