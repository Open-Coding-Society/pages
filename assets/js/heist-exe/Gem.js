...
/**
 * Gem extends the basic Coin class to support image/spritesheet rendering.
 * Instead of a colored circle, you can now display any image asset (gem, star, etc).
 * 
 * @example
 * const gemCoin = new Gem({
 *   id: 'gem-coin',
...
class Gem extends Coin {
	...
	loadImage() {
		...
		img.onload = () => {
			...
			console.log(`Gem image loaded: ${this.spriteImagePath}`);
		};
		img.onerror = () => {
			console.warn(`Failed to load Gem image: ${this.spriteImagePath}`);
			this.isImageLoaded = false;
		};
