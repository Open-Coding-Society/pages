  ...
  async function executeSelectiveSync() {
    ...
    try {
      ...
      const results = await Promise.all(
        events.map(event => 
          fetch(`${javaURI}/api/calendar/add_event`, {
            ...
            if (res.status === 401 || res.status === 403 || (res.redirected && res.url && res.url.includes('/login'))) {
              authFailed = true;
              return false;
            }
            return res.ok;
      ...
      if (successCount === events.length) {
        ...
        showToastNotification(`⚠ Synced ${successCount}/${events.length} events`, 'warning');
      } else {
      ...
      if (successCount === events.length) {
        ...
        showDateStatus(statusEl, `⚠ Synced ${successCount}/${events.length} events`, 'warning');
      } else {
      ...
      if (error instanceof TypeError && (error.message.includes('Failed to fetch') || error.message.includes('Load failed'))) {
        alert('Your session has expired. Please log in again to sync events.');
        window.location.href = SITE_BASEURL + '/login';
      } else {
  ...
  async function executeSelectiveRemove() {
    ...
    try {
      ...
      if (error instanceof TypeError && (error.message.includes('Failed to fetch') || error.message.includes('Load failed'))) {
        alert('Your session has expired. Please log in again to remove events.');
        window.location.href = SITE_BASEURL + '/login';
      } else {
  ...
  async function populateCalendarPreview(selectedItems, priority, courseName) {
    ...
    try {
      ...
      if (javaURI) {
        ...
        if (response.status === 401 || response.status === 403 || (response.redirected && response.url && response.url.includes('/login'))) {
