...
* **Student/Mentor view is chosen at login:** the login form's Student/Mentor toggle stores `ocsLoginRole` in `localStorage` (cleared on logout). Use `viewFor(roles)` from `assets/js/api/role-view.js` to decide the view — mentor view requires real `ROLE_MENTOR` *and* the choice not being `student`. The mentor sidebar is the student sidebar minus `bathroom_pass` (via the `exclude` param of `_includes/aesthetihawk/sidebar-list.html`) plus `_data/aesthetihawk_sidebar_mentor.yml`; Bathroom Pass UI (sidebar, toolkit button, profile facial registration) must stay hidden in the mentor view.
