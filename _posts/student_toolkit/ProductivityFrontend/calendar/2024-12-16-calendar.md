...
.event-priority-badge.p0 { background: #ef4444; color: white; }
.event-priority-badge.p1 { background: #f97316; color: white; }
.event-priority-badge.p2 { background: #eab308; color: white; }
.event-priority-badge.p3 { background: #22c55e; color: white; }
</style>
                ...
                .then((calendarEvents) => {
                    ...
                    if (calendarEvents !== null) {
                        calendarEvents.forEach(event => {
                            try {
                                // Get priority from event (default to P2 if not set)
                                const priority = event.priority || 'P2';

                                // Priority-based colors
                                const priorityColors = {
                                    'P0': '#fecaca', // Light red
                                    'P1': '#fed7aa', // Light orange
                                    'P2': '#fef08a', // Light yellow
                                    'P3': '#bbf7d0'  // Light green
                                };

                                let color = priorityColors[priority] || "#808080";

                                // Fall back to class-based colors if no priority
                                if (!event.priority) {
                                    if (event.class == "CSP") {
                                        color = "#3788d8";
