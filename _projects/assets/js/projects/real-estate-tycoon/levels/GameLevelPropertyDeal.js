...
class GameLevelPropertyDeal {
  constructor(gameEnv, config) {
    ...
    if (PortfolioManager.getCash() < candidates[0].price * 0.3) {
      this._noDeal = `Not enough funds! You need at least $${_fmt(candidates[0].price * 0.3)} to negotiate.`;
      this.classes = this._minimalClasses(path, config.backgroundImage);
      return;
    }

    // Pick the property for this deal
  ...
  _showDealScreen(won, message, finalPrice = 0, saved = 0) {
    ...
    overlay.innerHTML = `
      <div style="font-size:52px;font-weight:800;margin-bottom:14px;${won ? 'color:#ffd700;text-shadow:0 0 24px gold' : 'color:#ff5b5b'}">
        ${won ? '🏠 DEAL CLOSED!' : '❌ DEAL FAILED'}
      </div>
      <div style="font-size:18px;margin-bottom:8px;max-width:500px;line-height:1.5;white-space:pre-line">${message}</div>
      ${won ? `
        <div style="font-size:15px;color:#4eff91;margin-bottom:6px">Property added to portfolio!</div>
        <div style="font-size:14px;color:#64b5f6">Net Worth: $${_fmt(PortfolioManager.getNetWorth())}</div>
      ` : ''}
      <div style="margin-top:28px;font-size:14px;opacity:.7">Click or press any key to return to the city</div>
    `;
...
// CSS for floating text and shake animations
