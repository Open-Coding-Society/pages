...
Answer the student's question concisely and helpfully. Refer to their specific stats when relevant. Keep responses to 2-4 sentences unless more detail is genuinely needed.`;
  ...
  async showSkillRadar() {
    ...
    try {
      ...
      const avgScore = (orderedSkills.reduce((sum, s) => sum + s.value, 0) / orderedSkills.length);
      const gradeLabel = avgScore >= 4.5 ? 'A' : avgScore >= 4.0 ? 'A-' : avgScore >= 3.5 ? 'B+' : avgScore >= 3.0 ? 'B' : avgScore >= 2.5 ? 'C' : 'F';
      if (this.profilePanelView) {
        this.profilePanelView.update({ grade: `${gradeLabel} (${avgScore.toFixed(1)}/5)` });
      }
