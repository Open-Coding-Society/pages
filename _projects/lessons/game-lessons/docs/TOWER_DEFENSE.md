...
- implement a minimal `enemy.update` and `tower.update` and wire them into `Game.update()` as a concrete example,
- scaffold the `towers/` and `enemies/` folders and a demo wave spawner.