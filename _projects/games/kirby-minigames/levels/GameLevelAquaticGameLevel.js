...
const resolveKirbyMinigamesAssetPath = (basePath) => {
  ...
  return `${normalizedBasePath}/images/projects/kirby-minigames`;
};
...
class GameLevelAquaticGameLevel {
  constructor(gameEnv) {
    ...
    const assetPath = resolveKirbyMinigamesAssetPath(path);
    const roomParam = new URLSearchParams(window.location.search).get("room");
    const baseWorldWidth = gameEnv.innerWidth;
    const baseWorldHeight = gameEnv.innerHeight;
    const coopWorldWidthMultiplier = 1.65;
    let width = baseWorldWidth;
    let height = baseWorldHeight;
    ...
    const launchSeekLevel = () => {
      ...
      if (!primaryGame || typeof primaryGame.pause !== "function") {
        console.warn(
          "Aquatic could not launch Seek because the parent game control is missing.",
        );
        return;
      }
      ...
      requestAnimationFrame(() => {
        ...
        setTimeout(() => {
          ...
          try {
            ...
            console.warn(
              "Could not hide aquatic canvas state before launching Seek:",
              err,
            );
          }
    ...
    const getWorldDimensionsForPlayerCount = (playerCount = 1) => {
      const normalizedPlayerCount = Math.max(1, Number(playerCount) || 1);
      const widthMultiplier =
        normalizedPlayerCount > 1 ? coopWorldWidthMultiplier : 1;

      return {
        width: Math.round(baseWorldWidth * widthMultiplier),
        height: baseWorldHeight,
      };
    };

    const applyWorldSize = (playerCount = 1, options = {}) => {
      const { resizeObjects = false } = options;
      ...
      if (this.gameEnv?.game?.environment) {
        this.gameEnv.game.environment.innerWidth = nextDimensions.width;
        this.gameEnv.game.environment.innerHeight = nextDimensions.height;
      }

      this.gameEnv.size?.();

      if (this.gameEnv?.container) {
        this.gameEnv.container.style.position =
          this.gameEnv.container.style.position || "relative";
        this.gameEnv.container.style.width = `${nextDimensions.width}px`;
        this.gameEnv.container.style.minWidth = `${nextDimensions.width}px`;
        this.gameEnv.container.style.height = `${nextDimensions.height}px`;
        this.gameEnv.container.style.minHeight = `${nextDimensions.height}px`;
        this.gameEnv.container.style.overflowX = "auto";
      }

      if (resizeObjects && Array.isArray(this.gameEnv?.gameObjects)) {
    ...
    const slimeNpc = {
      ...
      interact: function () {
        ...
        if (!levelContext.canLeadStoryInteractions?.()) return;
        ...
        ocean: [
          {
            ...
            answer:
              "Plastics can entangle animals or be mistaken for food, which can cause injury, starvation, and toxic exposure.",
          },
          {
            question: "What can people do to reduce ocean trash?",
            answer:
              "Use reusables, sort waste correctly, avoid littering, and join local beach or river cleanups.",
          },
          {
            question: "Why are coral reefs important?",
            answer:
              "Coral reefs support biodiversity, protect coastlines from waves, and provide habitat for many fish species.",
          },
          {
            question: "What are microplastics?",
            answer:
              "Microplastics are tiny plastic fragments that enter water and food chains, affecting wildlife and ecosystems.",
          },
        ],
      },
      reaction: function () {},
    ...
    const lobbyRegistryKey = "aquatic_story_lobbies_v2";
    const defaultStarfishTotal = questState.firstQuest.starfishTotal;
    const defaultTrashTotal = questState.secondQuest.trashTotal;
    const defaultBossConfig = {
      maxHp: this.bossState.maxHp,
      shotCooldownMs: this.bossState.shotCooldownMs,
    };

    this.multiplayer = {
      enabled: Boolean(roomParam),
      coopActive: Boolean(roomParam),
      room: roomParam || null,
      lobbyName: roomParam || "",
      maxPlayers: 2,
      isHost: false,
      playerId: `aq_${Date.now()}_${Math.random().toString(36).slice(2, 10)}`,
      channelName: roomParam ? `aquatic_multiplayer_${roomParam}` : null,
      channel: null,
      heartbeatTimer: null,
      pruneTimer: null,
      registryTimer: null,
      ...
      displayName: (
        localStorage.getItem("aquatic_multiplayer_name") || "Diver"
      ).slice(0, 16),
      playerColor: "#7de2ff",
      storageHandler: null,
    };

    this.isTwoPlayerStoryMode = () =>
      this.gameMode === "story" &&
      !!this.multiplayer?.coopActive &&
      !!this.multiplayer?.room;

    this.applyWorldSize = applyWorldSize;

    const setGamePaused = (paused) => {
      if (this.gameEnv?.gameControl) {
        this.gameEnv.gameControl.isPaused = paused;
      }
    };

    const canLeadStoryInteractions = () => {
      if (!this.isTwoPlayerStoryMode()) return true;
      if (this.multiplayer.isHost) return true;
      showTopMenuNotice(
        "Only the lobby leader can talk to NPCs in 2 Player mode.",
      );
      return false;
    };
    this.canLeadStoryInteractions = canLeadStoryInteractions;

    const applyTwoPlayerBalance = () => {
      const coopActive = this.isTwoPlayerStoryMode();
      const q1 = questState.firstQuest;
      const q2 = questState.secondQuest;

      if (!q1.accepted || q1.collected === 0) {
        q1.starfishTotal = coopActive ? 14 : defaultStarfishTotal;
      }
      if (!q2.accepted || q2.collected === 0) {
        q2.trashTotal = coopActive ? 16 : defaultTrashTotal;
      }

      const nextMaxHp = coopActive ? 620 : defaultBossConfig.maxHp;
      const previousMaxHp = this.bossState.maxHp || nextMaxHp;
      const hpRatio = previousMaxHp > 0 ? this.bossState.hp / previousMaxHp : 1;
      this.bossState.maxHp = nextMaxHp;
      this.bossState.hp = this.bossState.active
        ? Math.max(1, Math.min(nextMaxHp, Math.round(nextMaxHp * hpRatio)))
        : nextMaxHp;
      this.bossState.shotCooldownMs = coopActive
        ? 980
        : defaultBossConfig.shotCooldownMs;
      this.bossState.damageMultiplier = coopActive ? 1.18 : 1;

      this.updateQuestHud?.();
    };
    this.applyTwoPlayerBalance = applyTwoPlayerBalance;

    const getPlayer = () =>
      this.gameEnv?.gameObjects?.find(
        (obj) => obj?.spriteData?.id === "playerData",
      );
    this.getLocalPlayer = getPlayer;

    const getStoryStarfishObjects = () =>
      (this.gameEnv?.gameObjects || []).filter((obj) =>
        obj?.spriteData?.id?.startsWith("starfish_"),
      );

    const clearStoryStarfish = () => {
      getStoryStarfishObjects().forEach((obj) => obj?.destroy?.());
    };

    const pruneLobbyRegistry = () => {
      try {
        const raw = localStorage.getItem(lobbyRegistryKey);
        const parsed = raw ? JSON.parse(raw) : {};
        const now = Date.now();
        const nextEntries = Object.entries(parsed || {}).reduce(
          (acc, [key, value]) => {
            if (!value || now - (value.updatedAt || 0) > 5000) return acc;
            acc[key] = value;
            return acc;
          },
          {},
        );
        localStorage.setItem(lobbyRegistryKey, JSON.stringify(nextEntries));
        return nextEntries;
      } catch (err) {
        return {};
      }
    ...
    const readLobbyRegistry = () => pruneLobbyRegistry();

    const writeLobbyRegistry = (entries) => {
      try {
        localStorage.setItem(lobbyRegistryKey, JSON.stringify(entries));
      } catch (err) {
        return;
      }
    };

    const updateLobbyRegistration = () => {
      if (!this.isTwoPlayerStoryMode()) return;
      const entries = readLobbyRegistry();
      entries[this.multiplayer.room] = {
        id: this.multiplayer.room,
        name: this.multiplayer.lobbyName || this.multiplayer.room,
        hostId: this.multiplayer.isHost
          ? this.multiplayer.playerId
          : entries[this.multiplayer.room]?.hostId || this.multiplayer.playerId,
        updatedAt: Date.now(),
        playerCount: Math.min(
          this.multiplayer.maxPlayers,
          this.multiplayer.remotePlayers.size + 1,
        ),
      };
      writeLobbyRegistry(entries);
    };

    const removeLobbyRegistration = () => {
      if (!this.multiplayer?.room) return;
      if (!this.multiplayer.isHost) return;
      const entries = readLobbyRegistry();
      delete entries[this.multiplayer.room];
      writeLobbyRegistry(entries);
    };

    const buildSharedStateSnapshot = () => ({
      questState: {
        firstQuest: { ...questState.firstQuest },
        secondQuest: { ...questState.secondQuest },
      },
      bossState: {
        active: this.bossState.active,
        introPlayed: this.bossState.introPlayed,
        combatReady: this.bossState.combatReady,
        hp: this.bossState.hp,
        maxHp: this.bossState.maxHp,
      },
      playerLock: this.playerLock,
      scene: questState.secondQuest.inSurface ? "surface" : "underwater",
      coopActive: this.isTwoPlayerStoryMode(),
      lobbyName: this.multiplayer.lobbyName,
    });

    const applySharedStateSnapshot = (snapshot) => {
      if (!snapshot) return;

      if (snapshot.questState?.firstQuest) {
        Object.assign(questState.firstQuest, snapshot.questState.firstQuest);
      }
      if (snapshot.questState?.secondQuest) {
        Object.assign(questState.secondQuest, snapshot.questState.secondQuest);
      }
      if (snapshot.bossState) {
        this.bossState.introPlayed = !!snapshot.bossState.introPlayed;
        this.bossState.active = !!snapshot.bossState.active;
        this.bossState.combatReady = !!snapshot.bossState.combatReady;
        if (typeof snapshot.bossState.maxHp === "number")
          this.bossState.maxHp = snapshot.bossState.maxHp;
        if (typeof snapshot.bossState.hp === "number")
          this.bossState.hp = snapshot.bossState.hp;
      }

      this.playerLock = !!snapshot.playerLock;
      if (snapshot.lobbyName) this.multiplayer.lobbyName = snapshot.lobbyName;
      applyTwoPlayerBalance();

      if (questState.secondQuest.inSurface) {
        setBackground(assetPath + "/Above the water.png");
        setWorldNpcVisibility(false);
        if (!this.surfaceTrashIds.length && !questState.secondQuest.completed) {
          spawnSurfaceTrash();
        }
      } else {
      ...
      if (
        questState.firstQuest.accepted &&
        !questState.firstQuest.completed &&
        !questState.secondQuest.accepted &&
        getStoryStarfishObjects().length === 0
      ) {
        spawnStarfish(questState.firstQuest.starfishTotal, true);
      }

      if (questState.firstQuest.completed) {
        clearStoryStarfish();
      }
    ...
    const drawRemotePlayer = (remote) => {
      if (!remote?.canvas || !remote?.ctx) return;

      const orientation = playerData.orientation || { rows: 1, columns: 1 };
      const pixels = playerData.pixels || { width: 16, height: 16 };
      const frameWidth = Math.max(
        1,
        Math.round(pixels.width / orientation.columns),
      );
      const frameHeight = Math.max(
        1,
        Math.round(pixels.height / orientation.rows),
      );
      const ctx = remote.ctx;

      remote.canvas.width = frameWidth;
      remote.canvas.height = frameHeight;
      ...
      const displayHeight = Math.max(
        54,
        localPlayer?.height ||
          this.gameEnv.innerHeight / playerData.SCALE_FACTOR,
      );
      const displayWidth = Math.round(
        displayHeight * (frameWidth / frameHeight),
      );
      remote.canvas.style.width = `${displayWidth}px`;
      remote.canvas.style.height = `${displayHeight}px`;
      remote.element.style.width = `${displayWidth}px`;
      remote.element.style.height = `${displayHeight}px`;

      if (!remote.spriteReady || !remote.spriteImage) {
        ctx.fillStyle = remote.color || "#7de2ff";
        ctx.beginPath();
        ctx.arc(
          frameWidth / 2,
          frameHeight / 2,
      ...
      const directionKey = playerData[remote.direction]
        ? remote.direction
        : "down";
      const directionData = playerData[directionKey] ||
        playerData.down || { row: 0, start: 0, columns: 1 };
      const frameCount = Math.max(
        1,
        directionData.columns || orientation.columns || 1,
      );

      if (remote.moving) {
        remote.frameTick = (remote.frameTick || 0) + 1;
        if (remote.frameTick % 5 === 0) {
          remote.frameIndex = ((remote.frameIndex || 0) + 1) % frameCount;
        }
      } else {
        remote.frameIndex = 0;
        remote.frameTick = 0;
      ...
      const sourceX =
        ((directionData.start || 0) + (remote.frameIndex || 0)) * frameWidth;
      const sourceY = (directionData.row || 0) * frameHeight;
      ...
      if (directionData.rotate) {
        ...
        ctx.rotate(directionData.rotate);
        ctx.translate(-frameWidth / 2, -frameHeight / 2);
      }
      ctx.drawImage(
        remote.spriteImage,
    ...
    const upsertRemotePlayer = (state) => {
      if (
        !state ||
        !state.playerId ||
        state.playerId === this.multiplayer.playerId
      )
        return;
      ...
      existing.moving =
        typeof state.moving === "boolean"
          ? state.moving
          : Math.hypot(
              (existing.x || 0) - (previousX || 0),
              (existing.y || 0) - (previousY || 0),
            ) > 1;
      ...
      if (!existing.element) {
        ...
        el.appendChild(spriteCanvas);
        el.appendChild(nameEl);
        document.body.appendChild(el);
        existing.element = el;
        existing.nameElement = nameEl;
        existing.canvas = spriteCanvas;
        existing.ctx = spriteCanvas.getContext("2d", {
          willReadFrequently: true,
        });
        existing.spriteImage = new Image();
        existing.spriteImage.onload = () => {
          existing.spriteReady = true;
          drawRemotePlayer(existing);
        };
        existing.spriteImage.onerror = () => {
          existing.spriteReady = false;
        };
        existing.spriteImage.src = playerData.src;
      }
    ...
    const handleMultiplayerMessage = (payload) => {
      ...
      if (payload.type === "quest_collect") {
        if (payload.quest === "starfish") {
          questState.firstQuest.collected = Math.min(
            questState.firstQuest.starfishTotal,
            questState.firstQuest.collected + (payload.amount || 1),
          );
          updateQuestHud();
          return;
        }

        if (payload.quest === "trash") {
          questState.secondQuest.collected = Math.min(
            questState.secondQuest.trashTotal,
            questState.secondQuest.collected + (payload.amount || 1),
          );
          updateQuestHud();
          if (
            questState.secondQuest.collected >=
            questState.secondQuest.trashTotal
          ) {
            transitionBackUnderwater(true);
          }
          return;
        }
      }

      if (payload.type === "scene_event") {
        if (payload.name === "surface_start") {
          transitionToSurface(true);
          return;
        }
        if (payload.name === "surface_return") {
          transitionBackUnderwater(true);
          return;
        }
        if (payload.name === "boss_start") {
          this.startMegalodonEncounter?.(true);
          return;
        }
        if (payload.name === "quest1_started") {
    ...
    const updateRemotePlayerRender = () => {
      ...
      this.multiplayer.remotePlayers.forEach((remote) => {
        ...
        remote.element.style.color = remote.color;
    ...
    const closeLobbyModal = () => {
      if (this.multiplayer.lobbyRefreshTimer) {
        clearInterval(this.multiplayer.lobbyRefreshTimer);
        this.multiplayer.lobbyRefreshTimer = null;
      }
      const modal = this.multiplayer.lobbyModal;
      if (modal) {
        const restorePaused = modal.dataset.restorePaused === "true";
        modal.remove();
        this.multiplayer.lobbyModal = null;
        if (!restorePaused) {
          setGamePaused(false);
        }
      }
    ...
    const joinLobby = ({ id, name, isHost = false }) => {
      if (this.gameMode !== "story") {
        showTopMenuNotice("2 Player mode is available in Story mode only.");
        return;
      }

      stopMultiplayer();
      this.multiplayer.enabled = true;
      this.multiplayer.coopActive = true;
      this.multiplayer.room = id;
      this.multiplayer.lobbyName = name;
      this.multiplayer.channelName = `aquatic_multiplayer_${id}`;
      this.multiplayer.isHost = isHost;
      this.multiplayer.displayName = (
        localStorage.getItem("aquatic_multiplayer_name") ||
        this.multiplayer.displayName ||
        "Diver"
      ).slice(0, 16);
      ...
      nextUrl.searchParams.set("room", id);
      window.history.replaceState({}, "", nextUrl.toString());

      applyWorldSize(this.multiplayer.maxPlayers, { resizeObjects: true });
      applyTwoPlayerBalance();
      updateQuestHud();
      startMultiplayer();
      document.getElementById("aquatic-top-menubar")?.remove();
      this.ensureTopMenuBar?.();
      closeLobbyModal();
      showTopMenuNotice(`Joined lobby ${name}.`);
    };

    const renderLobbyEntries = (container) => {
      if (!container) return;
      const entries = Object.values(readLobbyRegistry()).sort(
      ...
      if (!entries.length) {
        ...
        empty.textContent = "No active lobbies. Create one to start co-op.";
      ...
      entries.forEach((entry) => {
        ...
        const isFull = (entry.playerCount || 0) >= this.multiplayer.maxPlayers;
        item.textContent = `${entry.name} (${Math.min(entry.playerCount || 1, this.multiplayer.maxPlayers)}/${this.multiplayer.maxPlayers})`;
        Object.assign(item.style, {
          width: "100%",
          padding: "12px 14px",
          borderRadius: "12px",
          border: "1px solid rgba(136, 225, 255, 0.58)",
          background: isFull
            ? "rgba(34, 53, 69, 0.82)"
            : "rgba(5, 35, 56, 0.88)",
          color: isFull ? "rgba(225, 236, 242, 0.52)" : "#dff8ff",
          fontFamily: "'Press Start 2P', cursive, monospace",
          fontSize: "10px",
          textAlign: "left",
        ...
        item.onclick = () =>
          joinLobby({ id: entry.id, name: entry.name, isHost: false });
        container.appendChild(item);
      });
    };

    const openTwoPlayerLobbyModal = () => {
      if (this.gameMode !== "story") {
        showTopMenuNotice("Switch to Story mode before opening 2 Player mode.");
        return;
      }

      closeLobbyModal();
      const wasPaused = !!this.gameEnv?.gameControl?.isPaused;
      setGamePaused(true);

      const overlay = document.createElement("div");
      overlay.id = "aquatic-lobby-modal";
      overlay.dataset.restorePaused = wasPaused ? "true" : "false";
      ...
      Object.assign(panel.style, {
        ...
        padding: "24px",
        borderRadius: "18px",
        border: "2px solid rgba(125, 226, 255, 0.82)",
        background:
          "linear-gradient(180deg, rgba(7, 38, 61, 0.96), rgba(3, 18, 33, 0.96))",
        color: "#e6fbff",
      ...
      title.textContent = "2 Player Mode";
      title.style.fontSize = "18px";
      title.style.marginBottom = "12px";
      title.style.color = "#7de2ff";

      const subtitle = document.createElement("div");
      subtitle.textContent =
        "Pick a lobby or create a new one. Story progress and cutscenes stay synced for both divers.";
      ...
      nameInput.value = this.multiplayer.displayName;
      nameInput.maxLength = 16;
      nameInput.placeholder = "Diver name";
      Object.assign(nameInput.style, {
        width: "100%",
        marginBottom: "12px",
        padding: "12px",
        borderRadius: "12px",
        border: "1px solid rgba(136, 225, 255, 0.6)",
      ...
      leaveButton.textContent = "Leave Current Lobby";

      [createButton, closeButton, leaveButton].forEach((btn, index) => {
        Object.assign(btn.style, {
          flex: index === 2 ? "1 1 100%" : "1 1 0",
          padding: "12px 14px",
          borderRadius: "12px",
          border: index === 0 ? "none" : "1px solid rgba(136, 225, 255, 0.58)",
          background:
            index === 0
              ? "linear-gradient(90deg, #35b9ff, #5cf0ff)"
              : "rgba(5, 35, 56, 0.88)",
          color: index === 0 ? "#032030" : "#dff8ff",
          cursor: "pointer",
          fontFamily: "'Press Start 2P', cursive, monospace",
          fontSize: "10px",
        });
      ...
      createButton.onclick = () => {
        const displayName =
          (nameInput.value || "").trim().slice(0, 16) || "Diver";
        localStorage.setItem("aquatic_multiplayer_name", displayName);
        this.multiplayer.displayName = displayName;
        const lobbyId = `aquatic-${Date.now().toString(36)}`;
        joinLobby({
          id: lobbyId,
      ...
      closeButton.onclick = () => closeLobbyModal();
      leaveButton.onclick = () => {
        stopMultiplayer();
        closeLobbyModal();
        showTopMenuNotice("Left the 2 Player lobby.");
      };
      leaveButton.style.display = this.isTwoPlayerStoryMode()
        ? "block"
        : "none";
      ...
      buttonRow.appendChild(leaveButton);
      panel.appendChild(buttonRow);
      overlay.appendChild(panel);
      document.body.appendChild(overlay);

      this.multiplayer.lobbyModal = overlay;
      renderLobbyEntries(list);
      this.multiplayer.lobbyRefreshTimer = setInterval(
        () => renderLobbyEntries(list),
        700,
      );
    };

    this.openTwoPlayerLobbyModal = openTwoPlayerLobbyModal;
    ...
    const startMultiplayer = () => {
      ...
      panel.textContent = `2P Lobby: ${this.multiplayer.lobbyName || this.multiplayer.room}`;
      ...
      if (typeof BroadcastChannel !== "undefined") {
        ...
        this.multiplayer.storageHandler = (event) => {
          if (
            !event ||
            event.key !== `${this.multiplayer.channelName}_signal` ||
            !event.newValue
          )
            return;
          try {
      ...
      this.multiplayer.pruneTimer = setInterval(() => {
        ...
        if (this.multiplayer.uiPanel) {
          this.multiplayer.uiPanel.textContent = `2P Lobby: ${this.multiplayer.lobbyName || this.multiplayer.room} | Players: ${Math.min(this.multiplayer.remotePlayers.size + 1, this.multiplayer.maxPlayers)}/${this.multiplayer.maxPlayers} | ${this.multiplayer.isHost ? "Leader" : "Support"}`;
        }

        if (
          this.isTwoPlayerStoryMode() &&
          !this.multiplayer.isHost &&
          this.multiplayer.remotePlayers.size === 0
        ) {
          this.multiplayer.isHost = true;
          showTopMenuNotice(
            "Your teammate left. You are now the lobby leader.",
          );
        }
      }, 700);

      this.multiplayer.registryTimer = setInterval(() => {
        updateLobbyRegistration();
      }, 1000);
    ...
    const stopMultiplayer = () => {
      if (!this.multiplayer.enabled && !this.multiplayer.heartbeatTimer) return;
      ...
      if (this.multiplayer.registryTimer) {
        clearInterval(this.multiplayer.registryTimer);
        this.multiplayer.registryTimer = null;
      }
      ...
      removeLobbyRegistration();
      this.multiplayer.enabled = false;
      this.multiplayer.coopActive = false;
      this.multiplayer.room = null;
      this.multiplayer.lobbyName = "";
      this.multiplayer.channelName = null;
      ...
      applyWorldSize(1, { resizeObjects: true });
      applyTwoPlayerBalance();
      updateQuestHud();
      document.getElementById("aquatic-top-menubar")?.remove();
      this.ensureTopMenuBar?.();
    ...
    const updateQuestHud = () => {
      ...
      if (!q1.completed) {
        ...
        progress.textContent =
          "Starfish: " + q1.collected + " / " + q1.starfishTotal;
        status.textContent =
          q1.collected >= q1.starfishTotal
    ...
    const ensureQuestHud = () => {
      ...
      Object.assign(hud.style, {
        ...
        fontFamily: "'Press Start 2P', cursive, monospace",
        background:
          "linear-gradient(160deg, rgba(8, 45, 72, 0.92), rgba(3, 16, 34, 0.92))",
    ...
    const toggleChallengeLeaderboard = () => {
      ...
      showTopMenuNotice(
        isHidden ? "Leaderboard opened." : "Leaderboard hidden.",
      );
    };
    ...
    const clearChallengeStarfish = () => {
      this.challengeStarfishIds.forEach((id) => {
        const obj = this.gameEnv?.gameObjects?.find(
          (item) => item?.spriteData?.id === id,
        );
    ...
    const showChallengeWaveComplete = () => {
      const existing = document.getElementById(
        "aquatic-challenge-wave-complete",
      );
      if (existing) existing.remove();
      ...
      Object.assign(panel.style, {
        ...
        background:
          "linear-gradient(180deg, rgba(9,50,80,0.95), rgba(4,20,40,0.95))",
        color: "#e6fbff",
        fontFamily: "'Press Start 2P', cursive, monospace",
        textAlign: "center",
      });
    ...
    const ensureChallengeHud = () => {
      ...
      Object.assign(hud.style, {
        ...
        fontFamily: "'Press Start 2P', cursive, monospace",
        background:
          "linear-gradient(160deg, rgba(8, 45, 72, 0.94), rgba(3, 16, 34, 0.94))",
    ...
    const ensureTopMenuBar = () => {
      ...
      const twoPlayerBtn = createButton("2 Player Mode");
      twoPlayerBtn.onclick = () => openTwoPlayerLobbyModal();
      ...
      if (this.isTwoPlayerStoryMode()) {
        twoPlayerBtn.textContent = `Lobby: ${this.multiplayer.lobbyName || this.multiplayer.room}`;
        twoPlayerBtn.style.opacity = "0.92";
      }
    ...
    const transitionOverlay = (label) => {
      ...
      Object.assign(overlay.style, {
        ...
        inset: "0",
        background:
          "radial-gradient(circle at center, rgba(167, 241, 255, 0.18), rgba(1, 8, 18, 0.88))",
        display: "flex",
        alignItems: "center",
        justifyContent: "center",
        zIndex: "10030",
    ...
    const setWorldNpcVisibility = (visible) => {
      const ids = ["Mermaid", "Random Slime", "Shark", "SharkWingman"];
      if (!questState.secondQuest.completed) {
        ids.push("Kirby");
      }
      (this.gameEnv?.gameObjects || []).forEach((obj) => {
        if (
          !obj?.spriteData?.id ||
          !ids.includes(obj.spriteData.id) ||
          !obj.canvas
        )
          return;
      ...
      positions.forEach((pos, i) => {
        const trashData = {
          ...
          interact: function () {
            ...
            updateQuestHud();
            if (levelContext.isTwoPlayerStoryMode()) {
              broadcastMultiplayerMessage({
                type: "quest_collect",
                quest: "trash",
                amount: 1,
    ...
    const transitionToSurface = async (fromNetwork = false) => {
      const q2 = questState.secondQuest;
      if (q2.inSurface || q2.returning || q2.completed) return;

      if (!fromNetwork && levelContext.isTwoPlayerStoryMode()) {
        broadcastMultiplayerMessage({
          type: "scene_event",
          name: "surface_start",
        });
      ...
      try {
        ...
        if (player) {
          player.position.x = Math.min(
            this.gameEnv.innerWidth - player.width - 20,
            Math.max(20, player.position.x),
          );
    ...
    const transitionBackUnderwater = async (fromNetwork = false) => {
      const q2 = questState.secondQuest;
      if (!q2.inSurface || q2.returning || q2.completed) return;
    ...
    this.showSharkGameOver = () => {
      ...
      Object.assign(panel.style, {
        ...
        padding: "24px",
        background:
          "linear-gradient(180deg, rgba(8, 46, 74, 0.95), rgba(4, 18, 36, 0.95))",
    ...
    const ensureBossHud = () => {
      ...
      help.textContent =
        "Aim with mouse, Left Click: Shoot, Right Click: Trident Slash";
    ...
    const updateBossHud = () => {
      ...
      const ratio = Math.max(
        0,
        Math.min(1, this.bossState.hp / this.bossState.maxHp),
      );
      ...
      if (playerFill) {
        const pRatio = Math.max(
          0,
          Math.min(1, this.bossState.playerHp / this.bossState.playerMaxHp),
        );
    ...
    const shakeWorld = async (ms = 900) => {
      const bg = this.gameEnv?.gameObjects?.find(
        (obj) => obj?.constructor?.name === "GameEnvBackground",
      );
    ...
    const applyBossDamage = (damage, hitX, hitY) => {
      ...
      const showBossVictoryWindow = () => {
        ...
        const body = document.createElement("div");
        body.textContent =
          "The ocean is safe again. Continue to the next level.";
        Object.assign(body.style, {
          fontSize: "11px",
          lineHeight: "1.7",
          marginBottom: "18px",
    ...
    const applyPlayerDamage = (damage, hitX, hitY) => {
      ...
      const scaledDamage = Math.round(
        damage * (this.bossState.damageMultiplier || 1),
      );
      this.bossState.playerHp = Math.max(
        0,
        this.bossState.playerHp - scaledDamage,
      );
    ...
    const fireTridentShot = () => {
      ...
      const now = Date.now();
      if (now - this.bossState.lastShotAt < this.bossState.shotCooldownMs)
        return;
    ...
    const swingTrident = () => {
      ...
      const now = Date.now();
      if (now - this.bossState.lastMeleeAt < this.bossState.meleeCooldownMs)
        return;
      ...
      const angle = Math.atan2(
        this.bossState.mouseY - py,
        this.bossState.mouseX - px,
      );
      ...
      this.bossState.summons = this.bossState.summons.filter((minion) => {
        const mx =
          (minion.obj?.position?.x || 0) + (minion.obj?.width || 0) * 0.5;
        const my =
          (minion.obj?.position?.y || 0) + (minion.obj?.height || 0) * 0.5;
      ...
      this.bossState.enemyProjectiles = this.bossState.enemyProjectiles.filter(
        (projectile) => {
          ...
          const alignment =
            rocketDist > 0
              ? (rocketDx / rocketDist) * swingForwardX +
                (rocketDy / rocketDist) * swingForwardY
              : 1;
          ...
          if (projectile.element) {
            projectile.element.style.filter =
              "brightness(1.08) hue-rotate(165deg)";
          }
          if (projectile.flameElement) {
            projectile.flameElement.style.background =
              "linear-gradient(90deg, rgba(212,255,255,0.98) 0%, rgba(118,242,255,0.94) 38%, rgba(58,180,255,0.9) 72%, rgba(58,180,255,0) 100%)";
          }
    ...
    const updateBossCombat = () => {
      ...
      const startAbility = (name, durationMs) => {
        ...
        setBossSpriteSheet(
          state.megalodonAttackSheet,
          state.megalodonAttackPixels,
        );
      ...
      const distancePointToSegment = (px, py, x1, y1, x2, y2) => {
        ...
        const t =
          vv > 0 ? Math.max(0, Math.min(1, (wx * vx + wy * vy) / vv)) : 0;
        const cx = x1 + t * vx;
        const cy = y1 + t * vy;
      ...
      const commitLaser = () => {
        ...
        Object.assign(beam.style, {
          ...
          borderRadius: "9px",
          background:
            "linear-gradient(90deg, rgba(124,221,255,0.2), #33c8ff 35%, #7cf6ff 70%, rgba(164,245,255,0.32))",
          boxShadow:
            "0 0 20px rgba(51,200,255,0.95), 0 0 34px rgba(124,246,255,0.82)",
          pointerEvents: "none",
          zIndex: "10062",
      ...
      const commitRockets = () => {
        ...
        const spawnRocketBurst = (x, y) => {
          ...
          Object.assign(burst.style, {
            ...
            borderRadius: "50%",
            background:
              "radial-gradient(circle, rgba(255,243,168,0.98) 0%, rgba(255,156,74,0.95) 45%, rgba(255,110,43,0.28) 78%, rgba(255,110,43,0) 100%)",
        ...
        const rocketLaunches = [
          {
            angle: targetAngle - 0.54,
            spawnOffsetY: -22,
            homing: 0,
            speed: 5.1,
          },
          {
            angle: targetAngle - 0.18,
            spawnOffsetY: -8,
            homing: 0,
            speed: 5.5,
          },
          {
            angle: targetAngle + 0.16,
            spawnOffsetY: 10,
            homing: 0.09,
            speed: 5.2,
          },
          {
            angle: targetAngle + 0.48,
            spawnOffsetY: 24,
            homing: 0,
            speed: 4.9,
          },
        ...
        for (const launch of rocketLaunches) {
          ...
          Object.assign(flame.style, {
            ...
            background:
              "linear-gradient(90deg, rgba(255,244,184,0.98) 0%, rgba(255,194,92,0.95) 38%, rgba(255,125,44,0.92) 72%, rgba(255,84,28,0) 100%)",
      ...
      const chaseDistance =
        state.activeAbility === "bodySwing"
          ? 118
          : state.activeAbility === "rockets"
            ? 205
            : 162;
      const playerX = player.position.x + player.width * 0.5;
      const playerY = player.position.y + player.height * 0.5;
      ...
      const desiredCenterX =
        playerX - normalizedX * chaseDistance + orbitX * 22;
      const desiredCenterY =
        playerY - normalizedY * chaseDistance + orbitY * swimBob * 0.28;
      const clampedCenterX = Math.max(
        boss.width * 0.55,
        Math.min(this.gameEnv.innerWidth - boss.width * 0.55, desiredCenterX),
      );
      const clampedCenterY = Math.max(
        boss.height * 0.55,
        Math.min(this.gameEnv.innerHeight - boss.height * 0.55, desiredCenterY),
      );
      const isLaserCharging =
        state.activeAbility === "laser" && !state.abilityCommitted;
      ...
      const now = Date.now();
      const laserReady =
        now - state.lastAbilityAt.laser >= state.cooldowns.laser;
      const rocketsReady =
        now - state.lastAbilityAt.rockets >= state.cooldowns.rockets;
      const swingReady =
        now - state.lastAbilityAt.bodySwing >= state.cooldowns.bodySwing;
      ...
      if (state.activeAbility) {
        const currentAbilityDuration =
          abilityDurations[state.activeAbility] || abilityDurations.laser;
        const progress =
          1 -
          Math.max(
            0,
            (state.abilityEndsAt - now) / Math.max(1, currentAbilityDuration),
          );
        ...
        if (now >= state.abilityEndsAt) {
          ...
          state.abilityCommitted = false;
          setBossSpriteSheet(
            state.megalodonMoveSheet,
            state.megalodonMovePixels,
          );
        }
      }
      ...
      state.projectiles = state.projectiles.filter((p) => {
        ...
        if (p.flameElement) {
          const flicker =
            0.84 + Math.sin(p.life * 0.55 + (p.flamePhase || 0)) * 0.16;
        ...
        if (hitSummon >= 0) {
          ...
          const mx =
            (minion.obj?.position?.x || p.x) + (minion.obj?.width || 0) * 0.5;
          const my =
            (minion.obj?.position?.y || p.y) + (minion.obj?.height || 0) * 0.5;
          spawnHitEffect(mx, my, "#ffd18a");
          if (minion.obj?.destroy) minion.obj.destroy();
        ...
        if (
          p.life > 120 ||
          p.x < -20 ||
          p.x > this.gameEnv.innerWidth + 20 ||
          p.y < -20 ||
          p.y > this.gameEnv.innerHeight + 20
        ) {
          if (p.element) p.element.remove();
          return false;
        }
      ...
      state.summons = state.summons.filter((minion) => {
        ...
        const mag = Math.max(1, Math.hypot(dx, dy));
        const wobble =
          Math.sin(performance.now() * 0.006 + minion.wobblePhase) * 0.75;
        ...
        if (
          Math.hypot(
            playerX - (obj.position.x + obj.width * 0.5),
            playerY - (obj.position.y + obj.height * 0.5),
          ) < 55
        ) {
      ...
      state.enemyProjectiles = state.enemyProjectiles.filter((p) => {
        ...
        if (p.flameElement) {
          const flicker =
            0.86 + Math.sin(p.life * 0.72 + (p.flamePhase || 0)) * 0.18;
          p.flameElement.style.width = `${24 * flicker}px`;
        ...
        if (
          p.life > 200 ||
          p.x < -40 ||
          p.x > this.gameEnv.innerWidth + 40 ||
          p.y < -40 ||
          p.y > this.gameEnv.innerHeight + 40
        ) {
          if (p.element) p.element.remove();
          return false;
        }
      ...
      if (state.laserBeam) {
        ...
        if (beam.element) {
          ...
          if (now < beam.hitStartsAt) {
            ...
            beam.element.style.filter =
              "drop-shadow(0 0 8px rgba(124,246,255,0.65))";
          } else if (now <= beam.hitWindowUntil) {
            beam.element.style.opacity = "0.96";
            beam.element.style.filter =
              "drop-shadow(0 0 18px rgba(51,200,255,0.92))";
          } else {
    ...
    this.startMegalodonEncounter = async (fromNetwork = false) => {
      if (this.bossState.active || this.bossState.introPlayed) return;
      if (!fromNetwork && this.isTwoPlayerStoryMode()) {
        broadcastMultiplayerMessage({
          type: "scene_event",
          name: "boss_start",
        });
      ...
      await showBottomStoryDialogue(
        "Slime",
        "I got you the strongest power of the ocean so you can beat that stupid megalodon!",
      );
      await showBottomStoryDialogue(
        "Slime",
        "Listen carefully. Move your mouse to aim your trident.",
      );
      await showBottomStoryDialogue(
        "Slime",
        "Left Click throws a trident shot. Keep your distance and keep firing.",
      );
      await showBottomStoryDialogue(
        "Slime",
        "Right Click performs a close-range trident slash. Use it when the megalodon rushes you!",
      );
    ...
    this.retryBossEncounter = async () => {
      ...
      if (player) {
        const retryX = Math.max(
          28,
          Math.min(
            this.gameEnv.innerWidth * 0.28,
            this.gameEnv.innerWidth - player.width - 28,
          ),
        );
        const retryY = Math.max(
          52,
          Math.min(
            this.gameEnv.innerHeight * 0.3,
            this.gameEnv.innerHeight - player.height - 52,
          ),
        );
    ...
    const spawnStarfish = (countOverride, forceRespawn = false) => {
      ...
      const count =
        countOverride ||
        (isChallengeMode ? challengeState.waveTarget : q1.starfishTotal);
      const padding = 90;
      const minDist = 80;
      ...
      while (positions.length < count && attempts < 500) {
        ...
        const tooCloseToNpc =
          !isChallengeMode &&
          npcPositions.some(
            (npc) => Math.hypot(npc.x - x, npc.y - y) < minNpcDist,
          );
      ...
      positions.forEach((pos, i) => {
        const itemId = isChallengeMode
          ? `challenge_starfish_${challengeState.wave}_${i}`
          : `starfish_${i}`;
        const starfishData = {
          id: itemId,
          ...
          interact: function () {
            if (isChallengeMode) {
              ...
              updateChallengeHud();
              if (
                challengeState.collectedThisWave >= challengeState.waveTarget
              ) {
    ...
    const showQuestWindow = (autoConfirm = false) => {
      ...
      const body = document.createElement("div");
      body.textContent =
        "Collect all the lost starfishes scattered across the sea floor.";
      Object.assign(body.style, {
        fontSize: "12px",
        lineHeight: "1.6",
        marginBottom: "18px",
      ...
      confirm.textContent = autoConfirm ? "Dive In" : "Confirm";
      ...
      confirm.onclick = () => {
        overlay.remove();
        if (!questState.firstQuest.started) {
          spawnStarfish();
        }
        updateQuestHud();
        if (!autoConfirm && levelContext.isTwoPlayerStoryMode()) {
          broadcastMultiplayerMessage({
            type: "scene_event",
            name: "quest1_started",
          });
    ...
    const mermaidNpc = {
      ...
      interact: function () {
        ...
        if (q1.accepted) {
          ...
          this.dialogueSystem.showDialogue(
            "Please collect the starfishes scattered around the reef, and keep away from the shark.",
            "Mermaid",
            null,
          );
          return;
    ...
    const sharkWingmanNpc = {
      ...sharkNpc,
      id: "SharkWingman",
      INIT_POSITION: { x: 760, y: 420 },
    };
    ...
    this.classes = [
      ...
      { class: Npc, data: sharkWingmanNpc },
  ...
  initialize() {
    ...
    if (mermaid) {
      mermaid.setupCanvas = function () {
        ...
        const displayWidth = baseSize * aspect;
        const displayHeight = baseSize;
        ...
        this.canvas.style.zIndex =
          this.data && this.data.zIndex !== undefined ? this.data.zIndex : "10";
      };
    }
    ...
    const sharks = (this.gameEnv?.gameObjects || []).filter((obj) => {
      const id = obj?.spriteData?.id;
      return id === "Shark" || id === "SharkWingman";
    });
    ...
    if (sharks.length) {
      ...
      sharks.forEach((shark, sharkIndex) => {
        const baseSpeed =
          Math.max(
            1.2,
            Math.min(this.gameEnv.innerWidth, this.gameEnv.innerHeight) *
              0.0035,
          ) +
          sharkIndex * 0.15;
        shark._motion = {
        ...
        const resolveSharkCollisions = () => {
          ...
          const blockers = (this.gameEnv?.gameObjects || []).filter((obj) => {
            ...
            if (
              obj?.spriteData?.id === "Shark" ||
              obj?.spriteData?.id === "SharkWingman"
            )
              return false;
        ...
        shark.update = () => {
          ...
          const isWingman = shark.spriteData?.id === "SharkWingman";
          const shouldHideForScene =
            this.levelCompleted ||
            q2?.inSurface ||
            q2?.returning ||
            q2?.completed;
          const shouldHideForMode = isWingman && !this.isTwoPlayerStoryMode();

          if (shouldHideForScene || shouldHideForMode) {
          ...
          if (shark.position.y < 0) {
            ...
            shark.position.y + shark.height >
            this.gameEnv.innerHeight
          ) {
          ...
          shark.direction = directionFromVector(
            shark._motion.vector.x,
            shark._motion.vector.y,
          );
          shark.draw();
  ...
  destroy() {
    ...
    this.cleanupBossEncounter?.();
    document
      .querySelectorAll(".ai-npc-modal")
      .forEach((modal) => modal.remove());
    document
      .querySelectorAll(".ai-npc-container")
