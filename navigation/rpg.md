...
<script type="module">
    ...
    async function ensureBasePrefix() {
        ...
        for (const cand of uniq) {
            try {
                const testUrl = `${cand}/assets/js/GameEnginev1.1/Game.js?v=${Date.now()}`;
    ...
    async function loadEngine() {
        ...
        try {
            ...
            const advUrl = `${prefix}/assets/js/GameEnginev1.1/Game.js?v=${Date.now()}`;
    ...
    async function loadAdventureEngine() {
        try {
            const prefix = await ensureBasePrefix();
            const url = `${prefix}/assets/js/GameEnginev1.1/Game.js?v=${Date.now()}`;
